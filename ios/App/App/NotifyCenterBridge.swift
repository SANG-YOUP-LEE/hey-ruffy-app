// ios/App/App/NotifyCenterBridge.swift
import Foundation
import Capacitor
...
public class NotifyCenterBridge: CAPPlugin {
  ...
  @objc func purgeBase(_ call: CAPPluginCall) {
    ...
    let maxDelete = call.getInt("maxDelete") ?? 64
    ...
    func matches(_ r: UNNotificationRequest) -> Bool {
      ...
      // baseId 매칭: 완전 일치, 접두사 일치, userInfo.baseId, threadIdentifier 모두 허용
      let baseOK =
        id == baseId ||
        id.hasPrefix(baseId) ||
      ...
      if !gen.isEmpty {
        ...
        if id.contains("__g:\(gen)__") { return true }
        return false
      }
      return true
    }
