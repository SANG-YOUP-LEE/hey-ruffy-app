...
function isActiveOnAnyDay(r, s, e){
  ...
  while (cur<=last){ if (isActive(r,cur)) return true; cur.setDate(cur.getDate()+1); cur.setHours(0,0,0,0) }
  return false
}
function lastActiveDateInRange(r, s, e){
  const cur=new Date(e); cur.setHours(0,0,0,0)
  const first=new Date(s); first.setHours(0,0,0,0)
  while (cur>=first){ if (isActive(r,cur)) return new Date(cur); cur.setDate(cur.getDate()-1); cur.setHours(0,0,0,0) }
  return null
}
  ...
  getters: {
    ...
    occurrencesActivePeriod(){
      ...
      return items.filter(x => !x.isPaused && isActiveOnAnyDay(x, s, e))
        .map(x => {
          const d = lastActiveDateInRange(x, s, e) || s
          const k = dateKey(d)
          return { ...x, status: x?.statuses?.[k] ?? 'notdone', assignedDate: new Date(d), id: x.id }
        })
    },
    ...
    headerCounts(){
      const r = useRoutinesStore()
      const src = r.selectedPeriod==='T'
        ? [...(this.occurrencesActiveDay || [])]
